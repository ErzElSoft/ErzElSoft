...
const ServiceSection: React.FC = () => {
  ...
  return (
    <section id="services" className="w-full bg-[#f7f9fb] px-12 py-20">
            ...
            {services.map((service, index) => (
              <div key={index} className="border-b border-gray-200 pb-4">
                <div
                  ...
                  className={`cursor-pointer text-2xl font-semibold flex justify-between items-center transition-colors duration-300 mb-2 ${selectedTab === index ? 'text-orange-500' : 'text-gray-800 hover:text-orange-500'
                    }`}
                >
                  <span>{service.tab}</span>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className={`h-5 w-5 transform transition-transform duration-300 ${selectedTab === index ? 'rotate-180' : ''
                      }`}
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                ...
                <div
                  className={`transition-max-height duration-500 ease-in-out overflow-hidden ${selectedTab === index ? 'max-h-40' : 'max-h-0'
                    }`}
                >
      ...
      <section className="w-full bg-white px-12 py-20 min-h-[600px] mt-10 rounded-xl ">
